...
Spartan Touchdown is a 2D Michigan State University (MSU) themed scroller and platformer video game developed for MSU's 
Object-oriented Software Design CSE 335 class. The project was completed over about a 4 to 5-week period using a
method like scrum to keep our team on track. We used twice per week sprint meetings to explore design ideas, our
implementation, planning, and assignment of coding tasks, which kept out team on track. The final product was a fun game 
full of interesting challenges and power-ups that won our team 3rd place out of the class's roughly 50 teams.

--------------------

## Gameplay
The game is played using the left and right arrow keys to move the scroller left and right, and the space bar to perform
a jump. The scroller collides with all platforms and walls it touches. By collecting coins in a level, the player can 
increase their score. The player loses and the level is reset if the player falls out of the level or is defeated by an
enemy. The player wins and moves to the next level if the player collides with the level's goalpost.
...
### Enemies
There are a variety of enemies that can cause the player to lose and reset the level.
...
being snowy, the second in a forest, the third in the mountains, and the fourth in space. Each level has a scorecard, which 
increases when a coin is collected, but decreases with the passage of time. Each level also has a stopwatch indicating
how long the player has been in the level for. To beat a level, the player must make the scroller collide with the
level's goalpost, which is at the end of the level.

Levels can be automatically traversed by starting at level 1 and reaching the goalpost in each level. They can also be 
manually selected using the "Levels" tab in the toolbar.

WIDE LEVEL IMAGES HERE
QUICK LEVEL SELECTOR IMAGES HERE 
